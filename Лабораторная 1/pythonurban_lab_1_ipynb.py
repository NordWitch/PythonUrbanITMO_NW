...
Original file is located at
    https://colab.research.google.com/drive/1mtTiK8B9CYJGd3IhUiyop3sS5WjIqExD
...
def task_1(cities_to_rename):
  return [item.capitalize() for item in cities_to_rename] 
...
def task_2(cities_to_check,filter_):
  return [item for item in cities_to_check if item["population"] > filter_]
...
def task_4(cities, city):
  return [item.capitalize() for item in cities].count(city.capitalize())
...
def task_5_1(territories):
  territories_with_index = [item for item in territories] #попробовала включить сюда вычисления из цикла ниже, но не получилось. А это вообще возможно?
  for territory in territories_with_index:
    territory["index"]=sum(territory["green_zones"])/territory["territory_area"] #надо бы добавить проверку на 0, но мне лень :)
  return territories_with_index

territories_new = task_5_1(list_territories)

def task_5_2(territories):
  return sum([item["index"] for item in territories])/len(territories)

average_index = task_5_2(territories_new)

print(round(average_index,4))